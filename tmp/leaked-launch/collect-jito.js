// Atomic collect-and-rescue via Jito bundle.
// Single tx in the bundle:
//   - Funder pays fee + Jito tip
//   - 88cH signs collectCoinCreatorFee  -> drains vault to 88cH
//   - 88cH signs system transfer        -> 88cH balance to DESTINATION
// Bot can't insert between ix 2 and ix 3 (single tx, atomic).
// Bundle guarantees inclusion if leader is up.
...
const TRANSFER_BUFFER_LAMPORTS = parseInt(process.env.BUFFER_LAMPORTS || '5000', 10); // leave a tiny dust behind to avoid edge cases
...
(async () => {
  ...
  console.log('Leaked (collector):      ', leaked.publicKey.toBase58());
  console.log('Destination:             ', DESTINATION.toBase58());

  const vaultBalance = await sdk.getCreatorVaultBalance(leaked.publicKey);
  ...
  const leakedPreBal = await c.getBalance(leaked.publicKey, 'confirmed');
  const transferAmount = leakedPreBal + vaultLamports - TRANSFER_BUFFER_LAMPORTS;
